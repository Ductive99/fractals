...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/05/03 17:32:35 by esouhail          #+#    #+#             */
...
static int	mandelbrot_iter(double c_re, double c_im, t_app *app)
{
	double	z_re;
	double	z_im;
	double	tmp;
		...
		double scale)
{
	...
	while (x < app->width)
	{
		*p = app_color(app, mandelbrot_iter(re, im, app));
		++p;
		re += scale;
		++x;
	}
		...
		double scale)
{
	...
	while (x < app->width)
	{
		safe_put_pixel(app, x, y,
			app_color(app, mandelbrot_iter(re, im, app)));
		re += scale;
		++x;
	}
}
...
void	render_mandelbrot(t_app *app)
{
	...
	scale = 4.0 / ((double)min_int(app->width, app->height)
			* app->view.zoom);
